/*
 Leaflet.draw 1.0.5-alpha.7, a plugin that adds drawing and editing tools to Leaflet powered maps.
...
L.Draw.SimpleShape = L.Draw.Feature.extend({
	...
	_onMouseMove: function (e) {
		...
		if (this._isDrawing) {
			...
			var options = this._map.fire(L.Draw.Event.DRAWING, {layer: this._shape})
			if (options) {
				L.setOptions(this, options);
				this._tooltip.updateContent({text: "salam"});
				console.warn(options)
			} else {
				this._tooltip.updateContent({text: "end"});
			}
		}
	},
...
L.Draw.Rectangle = L.Draw.SimpleShape.extend({
	...
	_drawShape: function (latlng) {
		if (!this._shape) {
			...
			this._shape.setStyle({color: "red"})
		}
	},
	...
	_getTooltipText: function () {
		var tooltipText = L.Draw.SimpleShape.prototype._getTooltipText.call(this),
			...
			onDrawingText = this.options.onDrawingText,
			latLngs, area, subtext;

		if (shape) {
			latLngs = this._shape._defaultShape ? this._shape._defaultShape() : this._shape.getLatLngs();
			area = L.GeometryUtil.geodesicArea(latLngs);
			subtext = onDrawingText ? onDrawingText(latLngs) : showArea ? L.GeometryUtil.readableArea(area, this.options.metric) : '';
		}
